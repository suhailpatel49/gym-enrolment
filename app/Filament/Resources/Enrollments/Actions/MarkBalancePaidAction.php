...
class MarkBalancePaidAction
{
    public static function make(): Action
    {
        return Action::make('markBalancePaid')
            ...
            ->color('success')
            ->visible(fn (Enrollment $record): bool => auth()->check() && $record->has_balance && (float) $record->remaining_balance > 0)
            ->authorize(fn (): bool => auth()->user()?->isAdmin() || auth()->user()?->isStaff())
            ...
            ->failureNotificationTitle('This enrollment does not have a remaining balance.')
