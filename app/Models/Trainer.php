...
class Trainer extends Model
{
    ...
    public function monthlyLedger(): Collection
    {
        return $this->personalTrainingMembers()
            ...
            ->selectRaw('COUNT(CASE WHEN CAST(ROUND(gym_amount * 100) AS INTEGER) = 0 THEN 1 END) as full_payment_count')
            ->selectRaw('COUNT(CASE WHEN CAST(ROUND(gym_amount * 100) AS INTEGER) > 0 THEN 1 END) as gym_commission_count')
            ->selectRaw('COUNT(CASE WHEN CAST(trainer_payment_paid AS INTEGER) = 1 THEN 1 END) as paid_count')
            ->selectRaw('COUNT(CASE WHEN CAST(trainer_payment_paid AS INTEGER) = 0 THEN 1 END) as pending_count')
            ->selectRaw('SUM(CAST(ROUND(total_client_amount * 100) AS INTEGER)) as total_client_cents')
            ->selectRaw('SUM(CAST(ROUND(gym_amount * 100) AS INTEGER)) as total_gym_cents')
            ->selectRaw('SUM(CAST(ROUND(trainer_amount * 100) AS INTEGER)) as total_trainer_cents')
            ->groupBy('month')
            ->orderByDesc('month')
            ->get()
