...
## Gates
- reviewer_signoff: PASS for reviewed candidate `6b77b64c096560aa0301d670a4c74807efcde35b`
- qa_signoff: corrected final QA PASS for the same candidate (146 tests, 1,230 assertions)
- deploy_approval: explicitly renewed for the same candidate and the enrolment-only reset; completed 2026-09-21

## Third release attempt result

- deployed_candidate: `6b77b64c096560aa0301d670a4c74807efcde35b`; final deployment-record SHA is the docs-only direct child containing this section
- backup: `/var/backups/gym-enrolment/direct-combined-pt-ui-reset-third-20260921T081200Z`; database SHA-256 `605bf04166c1736c28f1bfe2a2af144f2d79086d199d9c5fc1fec64545b1ed4e`; integrity `ok`; foreign-key violations `0`; directories `0700`; files `0600`; base code, assets, non-secret service configuration, checksums, rollback instructions, and failed-activation evidence preserved
- rehearsal: the new backup was migrated on a disposable copy, snapshotted after migration, reset with the exact four-statement transaction, checked only by the dedicated post-reset verifier, then restored to the backup's exact SHA. The verifier excluded only the `enrollments` table and the `enrollments` sequence entry.
- migration_result: exactly the status and sessions migrations applied; status aggregate `active=1`, `cancelled=3`; all four allowed statuses remain in the constraint; historical sessions remain `NULL`; the two pre-existing money triggers and two sessions triggers are present; one `training_status,end_date` index is present
- reset_result: live enrollments `678 -> 0`, pending/jobs/failed jobs `0/0/0`, enrollment sequence absent, integrity `ok`, foreign-key violations `0`. Protected tables whose counts and row hashes remained exact: `cache`, `cache_locks`, `failed_jobs`, `job_batches`, `jobs`, `migrations`, `password_reset_tokens`, `personal_training_members`, `sessions`, `trainers`, `users`. Protected sequences remained exact for `failed_jobs`, `jobs`, `migrations`, `personal_training_members`, `trainers`, `users`.
- activation_incident: the first activation produced `/up=500` because the standard view-cache clear removed the dynamic health view while the runtime view directory was not writable by PHP-FPM. This real service failure triggered a full verified rollback to the backup/base code/assets and restored 678 enrollments. The runtime view-cache group writability was corrected and proven on the restored base before the authorized activation was repeated successfully; failed-release database evidence was preserved.
- verification: environment-free full PHPUnit `146/146` with `1,230` assertions; Vite 8.2.1 built 4 modules and all six manifest-referenced assets passed local and hosted byte/HTTP checks; sidebar CSS markers passed. Private Chromium with disposable SQLite/fake data rendered exactly `Overdue balances` and `Renewals due`, zero stat cards, readable Enrolment/PT sidebar groups, and PT Training status, Number of sessions `12001`, and Remarks. Hosted authenticated verification remains pending because no approved vault login was available.
- services: worker RUNNING; Nginx and PHP-FPM active; Nginx configuration valid; TLS verified; HTTP redirects to HTTPS; `/up=200`, `/enrol=200`, `/admin=302`; no external mail and no live test enrollment
- rollback_final: not active; the temporary rollback was fully verified, then superseded by the successful corrected activation
