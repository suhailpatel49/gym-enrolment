...
- downtime_and_risk: minimal schema work; avoid exposing the new resources/widget before migration, since they query the new tables. Schema rollback loses PT records. Renewal changes the current record only: no history, billing engine, scheduling, or automated processing. Active means the active flag is true, start date is today or earlier, and end date is today or later; future-start active records are Upcoming; pending-payment counts use that same set, and expiry counts include today through seven days ahead. Inactive trainers remain selectable so existing relationships remain editable.

## Gates
- reviewer_signoff: original implementation received independent read-only GPT-6 Astra review; no actionable correctness, security, or scope findings. No release signoff implied.
- qa_signoff: future-start regression tests failed before the fix. Focused tests passed (28 tests, 342 assertions); full PHPUnit suite passed (53 tests, 529 assertions). Both used `php vendor/bin/phpunit --bootstrap /tmp/pt-no-env-bootstrap.php` with a temporary bootstrap disabling Laravel environment-file loading, in-memory SQLite, a process-only APP_KEY, and an unused APP_CONFIG_CACHE path. `vendor/bin/pint --dirty --format agent` and `git diff --check` passed. `npm run build -- --config /tmp/pt-vite.config.mjs` passed (Vite 8.2.1, 3 modules; optional fontaine warning only); the temporary config imported the project config and redirected envDir to an empty temporary directory. No `.env` access, push, deployment, or shared database migration.
