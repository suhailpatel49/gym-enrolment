...
- expected_parent: `4934899e947f6a814e5ed50423d5ec89112d45db`
...
- hosted_admin_checks: authenticate with an approved existing admin account. Verify Dashboard mounts `OverdueBalances` and `RenewalsDue` only, with no `EnrollmentStats` or `PersonalTrainingStats` mini cards; verify both admin and staff retain their existing table permissions. At desktop and mobile widths in light and dark mode, verify sidebar brand, group labels/icons, normal items, active items, hover, and keyboard focus are readable while topbar/content contrast and mobile open/close controls remain unchanged. Verify PT status/session fields and current-scope behavior without creating enrolments or sending mail.
