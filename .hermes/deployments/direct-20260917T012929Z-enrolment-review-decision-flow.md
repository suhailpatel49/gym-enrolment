...
- queue_atomicity_gate: `queue.default` must be `database`, and that connection must use the database driver and resolve to the same database connection as `Enrollment`; approval fails closed before persistence otherwise. Verify with `php artisan config:show queue.default`, `php artisan config:show database.default`, and `php artisan config:show queue.connections.database` before activation. No new environment variables are introduced, and no configuration values are logged or exposed by the application error.
...
- cron: none
- workers: an active worker must consume the default `database` connection and its configured default queue. Before activation, in an approved non-production context using the non-external array mail transport, enqueue one harmless test enrollment confirmation, run the same default worker command used by the service, and verify the job leaves `jobs` without entering `failed_jobs`; remove the test enrollment afterward.
...
- qa_signoff: independent QA passed the correction base acceptance set at 13 tests/240 assertions; independent QA rerun of this correction remains pending
...
- focused_tests: final thirteen-file enrollment/tablet/migration/concurrency/UI/mail/PDF/terms run passed 63 tests with 473 assertions
- full_tests: `php artisan test --compact` passed 124 tests with 1,027 assertions without command-line environment overrides
...
- assets: `npm run build -- --config /tmp/direct-enrolment-review-decision-flow.jncPuy/vite.config.mjs` passed with Vite 8.2.1 and 4 modules transformed; the wrapper redirected `envDir` to an empty temporary directory and output to `/tmp/direct-enrolment-review-decision-flow.jncPuy/dist`
