...
## Dashboard follow-up scope
- base_commit: 6774320d3a44936d9d7043ddee4e74daf9a41baf
- dashboard: admins and staff see an exact warning-colored Pending approvals count linked to the enrollment index. The auto-discovered, full-width PendingApprovals table appears before overdue balances and renewals, lists only pending records newest first, defaults to five rows, and has a clear empty state.
- review_actions: View opens the existing enrollment detail page; the shared ApproveEnrollmentAction remains available to admins and staff. Approval removes the row on refresh and preserves existing exactly-once confirmation queueing and approver/time audit behavior.
- privacy: reference, member, package, and submission time are shared; email and mobile columns and their searches remain admin-only. No restricted personal or payment fields were added for staff.
- release_impact: dashboard follow-up adds no migrations, dependencies, environment variables, or worker changes. No push, deployment, or shared-database migration was performed.
...
- qa_signoff: local automated verification passed: 39 focused tests / 382 assertions; 59 full-suite tests / 518 assertions, including simultaneous approval and rollback. Pint, asset build, and git diff checks passed. Asset build emitted the existing optional fontaine warning. Laravel checks used isolated test databases with environment-file loading disabled; the asset build used a temporary environment-directory override. Staging QA pending

## Dashboard verification evidence
- TDD: new dashboard tests first failed on the missing pending stat, subheading, and widget; all nine new cases pass after implementation. Existing dashboard copy assertions were updated, and the existing statistics test still verifies all six approved-membership totals exclude pending records. No existing tests were removed.
- focused_command: `php artisan test --compact tests/Feature/PendingApprovalsDashboardTest.php tests/Feature/AdminDashboardTest.php tests/Feature/PendingEnrollmentTest.php tests/Feature/EnrollmentApprovalUiTest.php tests/Feature/EnrollmentApprovalTest.php tests/Feature/EnrollmentApprovalTransactionTest.php tests/Feature/StaffAccessTest.php` — 39 passed, 382 assertions.
- full_suite: `php artisan test --compact` — 59 passed, 518 assertions; no failures or skips.
- formatting: `vendor/bin/pint --dirty --format agent` — passed.
- assets: `npm run build -- --config /tmp/enrollment-dashboard-checks/vite.config.mjs` — passed, Vite 8.2.1, 3 modules transformed. Temporary config imports the unchanged project config and sets envDir to an empty temporary directory. Existing optional fontaine warning only.
- diff_check: `git diff --check` — passed.
- isolation: PHP checks used a temporary auto-prepend bootstrap that disables environment-file loading, explicit testing settings, SQLite in-memory databases, and the existing transaction tests' temporary SQLite files. No .env file was read, copied, or sourced.
